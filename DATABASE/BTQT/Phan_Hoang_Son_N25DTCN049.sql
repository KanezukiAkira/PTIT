...
-- Câu 1
SELECT employee_id AS 'Employee ID', full_name AS 'Employee Name', salary AS 'Monthly Salary' 
FROM employees;

-- Câu 2
SELECT full_name, salary * 12 AS 'Annual Salary' 
FROM employees;

-- Câu 3
SELECT full_name AS 'Name', hire_date AS 'Start Date' 
FROM employees;
...
-- Câu 5
SELECT SUM(salary) AS total_salary_fund 
FROM employees;
...
-- Câu 9
SELECT COUNT(employee_id) AS total_employees, SUM(salary) AS total_salary, AVG(salary) AS avg_salary, MAX(salary) AS max_salary, MIN(salary) AS min_salary 
FROM employees;
...
-- Câu 23
SELECT * FROM employees 
WHERE department_id IN (SELECT department_id FROM employees WHERE full_name = 'Nguyen Van An');
...
-- Câu 26
SELECT e.full_name, e.gender, d.department_name 
FROM employees e 
JOIN departments d ON e.department_id = d.department_id;

-- Câu 27
SELECT e.full_name, e.salary, d.department_name 
FROM employees e 
JOIN departments d ON e.department_id = d.department_id 
...
FROM employees e 
JOIN (SELECT department_id, MAX(salary) AS max_salary FROM employees GROUP BY department_id) max_dept 
ON e.department_id = max_dept.department_id AND e.salary = max_dept.max_salary;
